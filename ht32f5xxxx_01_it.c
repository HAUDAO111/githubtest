...
{
	int T;
	u32 TmpStatus1 = RESET;
	u32 TmpStatus2 = RESET;
	TmpStatus1 = HT32F_DVB_PBGetState(BUTTON_KEY1);
	TmpStatus2 = HT32F_DVB_PBGetState(BUTTON_KEY2);
	if (TmpStatus1 == RESET ) T = 1;
	if (TmpStatus2 == RESET ) T = 2;
	switch (T)
{
	/*moi lan ngat led se dao trang thai*/
		case 1: 
		{
		...
		/*moi lan ngat dich qua 1 bit led sang dan den dung trang thai led 0 (c14) sang thi cho quay lai led 2(c1) */ 
		case 2:
		{
			int y = HT_GPIOC->DOUTR;
			BFTM_ClearFlag(HT_BFTM0);
			HT_GPIOC->DOUTR  = 0xfffD;
			if (y==0x3fff) y = 0xfffD;
			else HT_GPIOC->DOUTR  = y >> 1;
			break;
		}
}
}
